...
8. （2026-06-13 移除）浏览器 smoke 测试矩阵：原 `node --test internal/modules/application/thoughtflow/service/web/app.browser.test.js` + 本机 Chrome headless + mock API server 覆盖 desktop/mobile 视口；本轮因浏览器自动化环境依赖复杂，与 `make e2e-test` / `make node-test` 覆盖重叠，决策删除。覆盖范围迁回 Node 组件测试 + API/SSE e2e 测试。
...
1. 当前按原生 HTML/CSS/JS 保持无构建链，已有 Node 组件测试（`make node-test`）和 API/SSE 端到端测试（`make e2e-test`）覆盖 Web 端关键路径。
...
各 PR 完成后 `make check` 全部绿：26/26 go + 35/35 node + 5/5 i18n + e2e-test。
...
- `make test` / `make node-check` / `make node-test` / `make node-test-i18n` / `make e2e-test` 通过。
- `git diff --check` 通过。
