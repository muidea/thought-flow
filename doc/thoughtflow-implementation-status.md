...
- **后端 API**：`/api/synthesis*` 路由下线，迁移到 `POST /api/compose/drafts`、`GET /api/compose/drafts`、`GET /api/compose/drafts/{id}`、`POST /api/compose/drafts/{id}/save`；草稿落盘目录改为 `compose/drafts/{draft_id}.yaml`，Thought `source` 枚举使用 `compose`。
- **专题刷新**：`POST /api/topics/{id}/rebuild` 下线，统一为 `POST /api/topics/{id}/refresh`；候选影响通过 `GET /api/topics/{id}/candidates` 返回 `[]TopicCandidateImpact`，覆盖 `capture_session` / `thought_reopen_session` / `thought` / `compose_draft` 4 个 source。
...
- **Web 路由**：`DEPRECATED_HASH_REDIRECTS` 重定向表与 `parseRoute` 中 `synthesis` / `thoughts` / `dashboard` 解析回退全部移除；`#/dashboard`、`#/thoughts`、`#/synthesis`、`#/jobs`、`#/settings` 旧 hash 不再被识别，访问时统一回退到 Overview。
- **i18n**：`NavDashboard` / `NavThoughts` / `NavJobs` / `NavSettings` / `ToastJobIDRequired` / `ToastDeprecatedRoute` 旧 key 清理；synthesis → compose、rebuild → refresh 全文更名。
...
- **历史文档保留**：本节之前的 `/api/synthesis*` / `synthesis/drafts/` / `source=synthesis` / `POST /api/topics/{id}/rebuild` 记录属于历史实现状态，按 todo 第 8 节口径保留但不再代表当前目标。

**本节收口动作的字面量**：本节中作为"已废弃的旧 hash/API"列举的字面量（`#/dashboard` / `#/thoughts` / `#/synthesis` / `#/jobs` / `#/settings` 与 `/api/synthesis*` / `synthesis/drafts/` / `source=synthesis` / `POST /api/topics/{id}/rebuild`）按 todo 第 8 节口径保留为"已收口动作"或"目标废弃说明"，不再代表当前可识别入口。

**Browser smoke 与残留 fixture 修复**（commit `5717798`，承接上一轮"无浏览器 skipped"的占位描述，本轮把 desktop/mobile/composition 链路在真实 Chrome 下跑通）：

- `app.js` `bind()` 中残留对 `#search-from` / `#search-to` / `#search-sort` / `#search-explain` 4 个旧 search 元素 ID 的引用，该批 ID 已在 #120 Search UI 收口时从 `index.html` 删除；残留代码导致 `boot()` 在 `null.addEventListener` 抛错，整套 chrome 烟雾卡在 `#system-status="Connecting"`。
- browser fixture 的 `/api/search` mock 仍返回 `items` 数组，但 `runSearch` 自 #115 起改读 `response.results`，桌面/移动 smoke 在搜索步骤拿不到结果项。
- browser fixture 缺少 `/api/topics/demo/candidates` route（#121 候选影响区新增），桌面/移动 smoke 在 topics detail 路径触发 404，console error 让 `errors === []` 断言失败。
...
- capture rich card 期望链接 `href="#/thoughts?id=thought-capture"`，#119 收口后应改 `#/notes?...`。
- 3 个 Go 文件（`models.go` / `compose service.go` / `topic service_test.go`）在前期收口 commit 留下对齐差异，`make fmt-check` 退出非 0。

**实现完成定义**：

- `rg "/api/synthesis|synthesis/drafts|source=synthesis|/api/topics/.*/rebuild|#/dashboard|#/thoughts|#/synthesis|#/jobs" internal cmd` 已无命中。
- `doc/` 中命中按 todo 第 8 节口径保留为历史实现状态 / 已收口动作 / 目标废弃说明，已在本节上文显式标注。
- `make test` / `make node-check` / `make node-test` / `make node-test-i18n` / `make e2e-test` 通过；`make browser-test` 在本机 Chrome 下 `12/14` 通过（desktop / mobile 烟雾矩阵 + capture composer 多轮会话 + capture PATCH 重渲染 + capture session lock 指示 + zh-CN/en-US 切换 + a11y + compose basket 持久化 + capture rich card），2 个用例因 Linux 主机无可用 Firefox / Safari 自动化而 skipped，0 fail。
- `git diff --check` 通过。
