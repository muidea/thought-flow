...
8. 浏览器 smoke 测试矩阵：
   ...
   - 测试矩阵显式声明 Chrome 目标(本轮 2026-06-13 业务范围收窄:用户明确说"我们只需要验证 Chrome")；Firefox / Safari / WebKit 矩阵条目已删除。
...
1. 当前按原生 HTML/CSS/JS 保持无构建链，已有 Node 组件测试、API/SSE 端到端测试和 Chrome 浏览器 smoke 矩阵(本轮 2026-06-13 业务范围收窄到 Chrome 唯一目标:用户明确说"我们只需要验证 Chrome")。Chrome 走 CDP（headless `--remote-debugging-port`）。本轮之前曾尝试 Firefox / WebKit Playwright 路径,见 §"跨浏览器收口历史"段。
...
各 PR 完成后 `make check` 全部绿：26/26 go + 35/35 node + 5/5 i18n + 12/12 browser smoke（chrome desktop/mobile + matrix outer 全部真跑；浏览器范围 2026-06-13 收窄到 Chrome 唯一目标）。
...
make browser-test                      # 12 通过 + 0 skip（chrome 唯一目标，本轮 2026-06-13 收窄）
...
- `make test` / `make node-check` / `make node-test` / `make node-test-i18n` / `make e2e-test` 通过；`make browser-test` 跑出 12 pass + 0 skip + 0 fail（chrome desktop/mobile 唯一目标；本轮 2026-06-13 业务范围收窄到 Chrome,Firefox / WebKit 矩阵条目已删除）。
- `git diff --check` 通过。

## 2026-06-13 业务范围收窄：浏览器 smoke 矩阵收窄到 Chrome 唯一目标

todo 第 8 节第 3 条要求"有浏览器环境时 `make browser-test` 通过"。本轮基于用户明确指示"我们只需要验证 Chrome",将 `app.browser.test.js` 浏览器矩阵从 `chrome / firefox / safari` 收窄到 `["chrome"]` 唯一目标。

### 收窄动作

1. `discoverBrowserTargets` 只返回 `chrome` 目标;`firefoxSkipReason` / `safariSkipReason` / `probePlaywright` / `playwrightAvailable` / `launchFirefox` / `launchSafari` / `findFirefox` / `isUnavailableSnapWrapper` / `PlaywrightPage` 全部删除(无引用)。
2. `connectPage` 简化(去掉 `kind === "playwright"` 分支,只剩 CDP 路径)。
3. `findChrome` 内联 `findExecutable` 为 PATH 遍历(被删的辅助函数不再被任何调用点需要)。
4. `package.json` / `package-lock.json` 删除(playwright devDep 唯一引用方 `PlaywrightPage` 已删);`node_modules/` 在 `.gitignore` 不入仓。
5. `browser smoke matrix declares cross-browser targets` 断言改为 `["chrome"]`。

### 验证

- `make browser-test`: 12 tests, 12 pass, 0 fail, 0 skip。
- chrome desktop (893ms) ✔ / chrome mobile (808ms) ✔
- matrix outer + 9 个独立 component test 全 ✔

### 与 todo 第 8 节第 3 条的关系

Chrome 是产品支持的唯一目标浏览器,本轮业务范围收窄后,todo 第 8 节第 3 条"有浏览器 → 通过"由 Chrome desktop + mobile 实测达成。

## 跨浏览器收口历史(2026-06-13 早段,后续被收窄覆盖)

> 本段保留作为本轮收窄前的跨浏览器尝试记录,供追溯。

todo 第 8 节第 3 条要求"有浏览器环境时 `make browser-test` 通过"。早段通过 Playwright 把 Firefox 从"探测后 skip"升级为"真跑 desktop + mobile"：
...
2. **代码改造**（`internal/modules/application/thoughtflow/service/web/app.browser.test.js`）：
   ...
   - `safariSkipReason` 保留 darwin-only 强约束:非 macOS 直接 skip;macOS 上走 `probePlaywright("webkit")` 真探测。
...
4. **测试侧最小化调整**（`app.browser.test.js` L317-322）:`usesGrid` 判定从 `width > 0 && width < 400` 收紧为 `width >= 130 && width < 250`。
5. **验证**:
...
6. **WebKit 处理**:WebKit 在 Linux 真跑通的尝试已穷尽(apt-get download + dpkg-deb -x + LD_LIBRARY_PATH + libSvtAv1Enc stub + PLAYWRIGHT_SKIP_VALIDATE_HOST_REQUIREMENTS 五重 workaround,最终 MiniBrowser 启动时仍报 `libgstcodecparsers-1.0.so.0: cannot open shared object file`)。早段未达成 WebKit 真跑通;后续本轮"业务范围收窄"决策**不再尝试装 WebKit**。
