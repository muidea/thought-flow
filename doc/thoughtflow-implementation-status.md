...
- `make test` / `make node-check` / `make node-test` / `make node-test-i18n` / `make e2e-test` 通过；`make browser-test` 在本机 Chrome 下 `12/14` 通过（desktop / mobile 烟雾矩阵 + capture composer 多轮会话 + capture PATCH 重渲染 + capture session lock 指示 + zh-CN/en-US 切换 + a11y + compose basket 持久化 + capture rich card）。firefox / safari 2 个用例因 Linux 主机不可用对应浏览器自动化而 `skipped`，0 fail；此 skip 符合 todo 第 8 节 line 155 "无浏览器时 skip 原因明确"的合规口径。
- `git diff --check` 通过。
