...
| 运行任务模型 | 后台任务、进度、错误和重试 | `.thoughtflow/jobs` + DuckDB 可选缓存 |
| 系统运维模型 | Git、配置、健康检查、事件游标 | `.thoughtflow/` + Git 仓库状态 |
...
| `runtime_path` | string | `.thoughtflow/` 运行态目录 | application |
...
1. 初始化工作区目录。
2. 校验 `thoughts/`、`topics/`、`.thoughtflow/` 可读写。
3. 按配置初始化 Git 仓库。
4. 提供系统状态查询基础信息。
...
2. 默认不提交 `.thoughtflow/` 和 DuckDB 文件。
3. 用户身份缺失时返回可理解错误。
