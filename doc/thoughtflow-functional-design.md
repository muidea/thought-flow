...
8. **可溯源**：每个搜索结果、专题内容、Compose 草稿结论都能反向链接到原始原子笔记。
...
5. **全量刷新**：`POST /api/topics/:id/refresh` 触发后，基于"全部 Thought + 全部未 commit scratchpad + 全部从 Thought 重新发起的会话 + Compose 草稿"刷新正式/候选/冲突三态。
...
- `POST /api/compose/drafts`：创建 Compose 草稿。
- `GET /api/compose/drafts`：查询 Compose 草稿列表。
- `GET /api/compose/drafts/{id}`：查询 Compose 草稿详情。
- `POST /api/compose/drafts/{id}/save`：保存 Compose 草稿为 Thought。

---

### 3.6 git-sync（Git 同步运行单元）
...
Web 端按 `Overview / Capture / Notes / Search / Topics / Compose` 六个主入口组织接口调用，Settings 作为全局 Drawer。接口契约必须服务页面职责，不把调试字段或跨域表单塞回主流程。

| 页面 | 主要接口 | 交互边界 |
|---|---|---|
| Overview | `/api/system/status`、`/api/system/metrics`、`/api/events` | 只展示健康、最近活动和快捷入口 |
| Capture | `/api/capture/sessions*`、`/api/thoughts/{id}/reopen-session` | 多轮对话内完成输入、上下文、预览和归档 |
| Notes | `/api/thoughts/{id}`、retry refine、reopen-session | 以阅读和状态查看为中心，可加入 Compose |
| Search | `/api/search`、`/api/thoughts/{id}` | 关键词搜索和内容相关筛选，不承载 reindex 或运行调试 |
| Topics | `/api/topics*`、candidates、refresh、weave | 专题正文、候选影响确认和规则维护 |
| Compose | `/api/compose*`、`/api/thoughts/{id}` | 多来源整理篮、草稿编辑、保存为 Thought |
| Settings Drawer | system、jobs、events、reindex、external config | 系统治理、运行状态和高风险操作确认 |
...
| `GET` | `/api/topics/{id}/candidates` | — | `[]TopicCandidateImpact` | — |
...
### 6.5 SearchResultView 投影

`GET /api/search` 的 Web 投影以内容相关性为中心：
...
内部 score 可用于排序，但默认不进入响应投影；需要排障时通过 Settings 或日志查看。
...
### 6.7 ComposeBasket / ComposeDraft

Compose 来源篮是临时交互状态，保存为 Thought 前不写入知识资产：

```go
type ComposeSource struct {
    SourceType string            `json:"source_type"` // thought|search_result|topic_section|capture_session
    SourceID   string            `json:"source_id"`
    Title      string            `json:"title"`
    Snippet    string            `json:"snippet"`
...
type ComposeDraft struct {
    ...
    SavedThoughtID string          `json:"saved_thought_id,omitempty"`
}
```
