...
持久事实源以 `thoughts/` 和 `topics/` 下的 Markdown/YAML 为主；`workspace.data_dir` 是本地运行态目录，不应作为长期事实源提交。
...
数据目录由 `workspace.data_dir` 明确定义。启动时会校验：

1. `config-dir` 不能等于 `workspace.data_dir`。
2. `config-dir` 不能放在 `workspace.data_dir` 下。
3. `workspace.data_dir` 不能放在 `config-dir` 下。

推荐部署形态：

```text
/etc/thoughtflow/application.toml   配置文件
/var/lib/thoughtflow/data/          运行态数据目录
```
...
1. `search.duckdb_path` 为相对路径时，会解析到 `workspace.data_dir` 下。
...
5. 配置目录和数据目录应保持物理分离。配置目录存放 `application.toml`，数据目录存放 jobs、logs、DuckDB 等运行态文件。
